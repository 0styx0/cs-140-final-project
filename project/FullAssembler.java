...
public class FullAssembler implements Assembler {
	...
	public int assemble(String inputS, String outputS, StringBuilder error) {
		...
		try (Scanner inp = new Scanner(new File(inputS))) {
			...
			boolean blankAlreadyFound = false;
			...
			while(inp.hasNextLine()) {
				...
				if(line.trim().length() == 0/*check for blank line here*/) {
					//TODO: keep in mind blank lines are legal as long as there is no non-blank lines that appear after the blank lines
					//if we have not found a blank line:
						//1. set the blankLineFound to true, indicating we have encountered one
						//2. be sure to store the current lineNum inside firstBlankLineNum, so we have that information if we need it later.
					if(!blankLineFound/*check if a blank line has not been found yet*/) {
						blankLineFound = true;
				...
				//otherwise we are looking at a non blank line
					...
					if(!blankAlreadyFound && blankLineFound/* check if we have found a blank line here*/) {
						...
						//1. append an appropriate error message to the String builder error, using the append() method
						//2.set retVal to the line number of the illegal blank line (recall we stored this blank line number above)
						//3. set blankLineFound back to false, so we can search for more illegal blank lines if they are present
					}
					...
					if(line.charAt(0) == ' ' || line.charAt(0) == '\t'/*check for illegal white space at start of line*/) {
					...
					if(line.trim().toUpperCase().equals("DATA")/*check if this line contains the DATA delimiter*/) {
						...
						if(!line.trim().equals("DATA")/*check if the DATA delimiter is not all upper cased*/) {
							error.append("\nLine does not have DATA in upper case");
							retVal = lineNum;
							// append to error and update retVal
						}
						...
						//so we have a duplicate DATA delimiter. this is an error
							...
							error.append("Duplicate DATA delimiter at the current line");
							// report Duplicate DATA delimiter at the current line
						}
					}
		...
		for(String line : code) {
			...
			//otherwise, we are dealing with a legal mnemonic
				...
				// check if mnemonic is supposed to take no arguments
				...
				if(Instruction.NO_ARG_MNEMONICS.contains(parts[0])/*check if this mnemonic doesn't take arguments*/) {
					// if parts.length is greater than 1, append an error message, explaining this mnemonic cannot take arguments, and update retVal
					if (parts.length > 1) {
				...
				//otherwise, the mnemonic should have an argument
					...
					// if parts.length is greater than 2, there are too many arguments present. append to error and update retVal
					if(parts.length > 2/*check for too many args*/) {
						error.append("\nError on line " + lineNum + ": this mnemonic has too many arguments");
						retVal = lineNum;
					}
					// else if parts.length is less than 2, the mnemonic is missing an argument. append to error and update retVal
					else if(parts.length < 2/*check for missing argument*/) {
					...
					//otherwise, there is a correct number of arguments
						...
						//make sure you remember to get rid of the 'N' from the argument as we did above.
							...
							//also make sure to get rid of the 'N' from the argument
							parts[1] = parts[1].substring(1);
						}
						// do a similar check for 'J'.
						...
						else if (parts[1].charAt(0) == 'J'/*check for J*/) {

							if(!Instruction.JMP_MNEMONICS.contains(parts[0].toUpperCase())) {
								error.append("Error on line " + lineNum +
										": this mnemonic does not allow jump mode\n");
								retVal = lineNum;
							}
							//also make sure to get rid of the 'J' from the argument
							parts[1] = parts[1].substring(1);
						}
		...
		for(String line : data) {
			...
			// if there are less than 2, report that data entry has too few numbers and update retVal.
			//this will be similar to how we checked in the code error parsing
			if(parts.length < 2/*check for too few numbers*/) {
				error.append("data entry has too few numbers");
				retVal = lineNum; // not sure if I should do this
			}

			// else if there are more than 2, report that data entry has too many numbers and update retVal.
			//this will be similar to how we checked in the code error parsing
			else if(parts.length > 2/*check for too many numbers*/) {

				error.append("data entry has too many numbers");
				retVal = lineNum;
			}
			...
			//we just need to see if they are both hexadecimal
				...
				// similar to what we did in the code error checking,
